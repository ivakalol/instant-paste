...
const MediaViewer: React.FC<MediaViewerProps> = ({ item, onClose, showToast }) => {
  ...
  const handleDownload = () => {
    if (!isComplete || !item.content) {
      showToast('Full version not yet available. Please wait for the transfer to complete.', 'info');
      return;
    }
  ...
  return (
    <div className="media-viewer-overlay" onClick={onClose}>
      ...
      <div className="media-viewer-content" onClick={(e) => e.stopPropagation()}>
        {item.type === 'image' ? (
          <img src={displayContent} alt={item.name || 'Image'} className="media-viewer-media" />
        ) : (
          <video src={displayContent} className="media-viewer-media" controls autoPlay />
        )}
      </div>

      <div className="media-viewer-toolbar" onClick={(e) => e.stopPropagation()}>
        <button onClick={handleDownload} className="media-viewer-btn" disabled={!isComplete}>
        ...
        {item.type === 'image' && (
          <button onClick={handleCopy} className="media-viewer-btn" disabled={!isComplete}>
            <svg width="18" height="18" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" viewBox="0 0 24 24">
